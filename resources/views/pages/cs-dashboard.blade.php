...
new
#[Layout('layouts.app', ['title' => 'CS Dashboard'])]
class extends Component {
    ...
    {
        if (trim($this->message) === '' || !$this->activeConversationId) return;
...
>
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6">
        ...
        <div class="flex gap-4" style="height: calc(100vh - 200px);">
            ...
            <div class="w-80 flex-shrink-0 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    @empty
                        <div class="p-8 text-center text-gray-400">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12 mx-auto mb-2 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                            </svg>
            ...
            <div class="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                @if($activeConversationId)
                    ...
                    <div class="p-4 border-b border-gray-100 flex items-center justify-between bg-white">
                        ...
                        <div class="flex items-center gap-2">
                            @if($activeConv && $activeConv['status'] === 'open')
                                <button
                                    wire:click="closeConversation({{ $activeConversationId }})"
                            ...
                            @else
                                <button
                                    wire:click="reopenConversation({{ $activeConversationId }})"
                    ...
                    <div
                        x-ref="csChat"
                        x-init="$nextTick(() => $refs.csChat.scrollTop = $refs.csChat.scrollHeight)"
                    ...
                    >
                        @forelse($messages as $msg)
                            ...
                            <div class="flex flex-col {{ $isCs ? 'items-end' : 'items-start' }}">
                                <div class="px-4 py-2.5 rounded-2xl max-w-[60%] text-sm
                                    {{ $isCs
                                        ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-br-md'
                                        : 'bg-white text-gray-800 border border-gray-200 rounded-bl-md shadow-sm' }}">
                                    @unless($isCs)
                                        <span class="block text-xs font-semibold text-blue-500 mb-1">{{ $msg['user'] }}</span>
                                    @endunless
                                    <span>{{ $msg['text'] }}</span>
                                </div>
