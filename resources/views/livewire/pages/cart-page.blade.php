        ...
        @if($items->isEmpty())
            ...
            <div class="bg-white rounded-3xl p-16 text-center shadow-xl border border-gray-100">
                <div class="w-28 h-28 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6">
                    <svg class="w-14 h-14 text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" />
                    </svg>
                </div>
        ...
        @else
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-4">
                    {{-- Header --}}
                    <div class="bg-white/80 backdrop-blur-xl rounded-2xl px-6 py-4 shadow-sm border border-white flex items-center justify-between">
                            ...
                            <div class="flex gap-4 p-5">
                                ...
                                <div class="flex-1 min-w-0 flex flex-col">
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="min-w-0">
                                            @if($product->brand)
                                                <span class="text-[10px] font-bold text-[#4E44DB] uppercase tracking-wider">{{ $product->brand->name }}</span>
                                            @endif
                                            ...
                                            <div class="flex flex-wrap gap-1.5 mt-1.5">
                                                @if($variant->condition)
                                                    <span class="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-gray-100 text-gray-600">{{ $variant->condition }}</span>
                                                @endif
                                                @if($variant->color)
                                                    <span class="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-blue-50 text-blue-600">{{ $variant->color }}</span>
                                                @endif
                                                @if($variant->storage)
                                                    <span class="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-purple-50 text-purple-600">{{ $variant->storage }}</span>
                                                @endif
                                                @if($variant->ram)
                                                    <span class="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-600">{{ $variant->ram }} RAM</span>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-auto pt-3 flex items-center justify-between">
                                        ...
                                        {{-- Qty Controls --}}
                                        <div class="flex items-center gap-0 bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                                            ...
                                            <span class="w-10 text-center text-sm font-bold text-gray-800 select-none">{{ $item->qty }}</span>
                                    ...
                                    @if($variant->stock <= 3 && $variant->stock > 0)
                                        <p class="text-[10px] font-semibold text-amber-500 mt-1.5 flex items-center gap-1">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-[88px]">
                        ...
                        <div class="border-t border-gray-100 mt-5 pt-5">
                            <div class="flex justify-between items-center mb-5">
                                ...
                                <span class="font-black text-xl text-[#4E44DB]">Rp {{ number_format($totalPrice, 0, ',', '.') }}</span>
                            </div>
